...
- [x] `validation/harness.py` — iterate CSV؛ PnL از `ExecutionEngine` نه trade_simulator مستقیم
- [x] `execution/simulated.py` — `SimulatedExecutionEngine` + `FillModel`
- [x] `execution/risk_gate.py` — `ExecutionRiskGate` (pre-trade)
- [x] `state/` — runtime cycle phases per [state-risk-contract.md](../architecture/state-risk-contract.md)
- [x] `validation/metrics.py`:
...
- [x] `validation/report.py` — گزارش با تمرکز بر Decision quality
- [x] DB models: `DecisionRecord`, `BacktestRun`, `SimulatedTrade`, `event_log`, `feature_sets`, `state_snapshots`
- [x] Alembic migrations
- [x] `scripts/run_validation.py` — CLI
- [x] `replay/engine.py` — **strict replay** از `event_log` (MVP)
- [x] `replay/timeline.py` — timeline per `correlation_id`

### معیارهای قبولی فاز

**Engine quality:**
- [x] Decision log + `state_snapshot_id` برای 100% چرخه‌ها ذخیره می‌شود
- [x] rejection breakdown گزارش می‌شود
- [x] `event_log` chain کامل Market → Execution برای هر cycle

**Outcome (با Mock یا اولین Provider):**
- [x] harness روی fixture CSV اجرا می‌شود
- [x] metrics قابل reproduce
...
- [x] `providers/base.py` — `BaseSignalProvider` implements protocol
- [x] `providers/registry.py` — register / discover
- [x] `providers/ema_crossover.py` — تفسیر `features.flags["ema_cross_bullish"]`
- [x] `providers/rsi_divergence.py` — تفسیر `features.indicators["rsi_14"]` + `ProviderRationale`
- [x] `config/providers/*.yaml` — پارامترها
- [x] Unit test هر provider — ورودی `FeatureSet` mock، **بدون** OHLCV
- [x] Integration test: Runtime + real providers + validation harness

### خروجی / معیار قبولی
- [x] افزودن provider سوم بدون تغییر Engine/Runtime
- [x] `enabled: false` در config → provider skip می‌شود
- [x] validation harness با providers واقعی اجرا می‌شود
...
### Infrastructure
- [x] Event Model — chain کامل در `event_log`
...
- [x] Provider بدون محاسبه اندیکاتور

### Validation
- [x] harness با engine metrics
- [x] outcome metrics (PnL) روی تصمیمات approved

### Providers
- [x] حداقل 2 provider plug-in
